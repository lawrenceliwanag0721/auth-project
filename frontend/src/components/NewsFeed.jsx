...
function PostCard({ post }) {
  ...
  const actions = [
    ...
    { key: "comment", icon: MessageCircle, count: post.comments },
  ...
  return (
    <div className="border border-black/30 rounded-lg p-4 flex flex-col gap-3 w-full">
      <div className="flex flex-row items-center gap-3">
        ...
        <div className="flex flex-col leading-tight">
          <span className="text-sm text-black">{post.name}</span>
          <span className="text-xs text-black/50">{post.handle} · {post.time}</span>
        </div>
      </div>

...
export default function PostDetail({ post, replies = mockPosts, postId }) {
  ...
  return (
    <>
      <div className="w-full max-w-2xl flex flex-col gap-8">
        <Link
          href="/"
          className="bg-white/70 backdrop-blur-md text-black/60 hover:text-black transition-colors sticky top-0 p-4 hover:bg-zinc-100"
        >
          ← Post
        </Link>
        ...
        <div className="w-full max-w-2xl flex flex-col gap-2">
          {replies.map((reply) => (
            <PostCard key={reply._id} post={reply} isReply={true} />
          ))}
        </div>
      </div>
      ...
      <div className="fixed bottom-0 left-0 w-full flex justify-center bg-white pb-8 px-8">
        <div className="w-full max-w-2xl">
          <ReplyComposer postId={postId} onCreateReply={addReply}/>
        </div>
      </div>
    </>
  )
